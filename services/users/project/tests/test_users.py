...
class TestUserService(BaseTestCase):
    ...
    def test_single_user(self):
        ...
        user = add_user(username="mk", email="test@test.com")
        with self.client:
            response = self.client.get(f'/users/{user.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn("mk", data["data"]["username"])
    ...
    def test_all_users(self):
        ...
        with self.client:
            ...
            self.assertIn('mk', data['data']['users'][0]['username'])
            self.assertIn('test@test.com', data['data']['users'][0]['email'])
            self.assertIn('mk2', data['data']['users'][1]['username'])
